...
def load_settings(self):
    """
    Loads Settings and their submodules from the JSON file
    ...
    if not os.path.exists(self.settings_json_path):
        print(f"[Sidebar] JSON not found: {self.settings_json_path}")
        return
    ...
    try:
        for idx, module in enumerate(data.get("Settings", [])):
                ...
                for index, sub in enumerate(module.get("submodules", [])):
                    if sub.get("enabled") and sub.get("licensed"):
                        ...
                        try:
                            ...
                            self.frames[name].autosave_toggled.connect(self.parent().update_footer)
                            ...
                            continue
        interfaces.default_sub_modules = interfaces.sub_modules
        self.submodule_sidebar.addStretch()

    except Exception as e:
        print(f"[Sidebar] Error loading settings: {e}")
        QMessageBox.critical(None, "Error", f"Failed to load settings:\n{str(e)}")