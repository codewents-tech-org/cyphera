...
class AttackTree_riskcontrol_Update():
    def __init__(self, attack_tree):
        ...
        for (Node_ID, Parent_ID, Node_Type, Text, AF_Value, AF_Text, RF_Value, RF_Text, Gate_Type, Values, Image_Type, x, y) in available_attack_tree:
            node_id = f"{attack_tree}_node_{self.node_counter}"
            if Node_Type == 'riskcontrol head':
                self.riskcontrol_tree_added = True
                self.tree_parent_list.append(Parent_ID)
                control = Text.strip().split(' ')[0]
                available_riskcontrol = DB.execute_db(f"SELECT * FROM riskcontrol_tree WHERE Node_ID like '{control}_node%'")
                # print(available_riskcontrol)
                node_map = {}
                control_head_node = ''
                for node_data in available_riskcontrol:
                        cir_id, control_parent_id, node_type, text, value, af_text, gate_type, values, image_type, c_x, c_y = node_data
                        node_map[cir_id] = {
                        ...
                        if control_parent_id != '': node_map[control_parent_id]["children"].append(cir_id)
                        else: control_head_node = cir_id
                if available_riskcontrol:
                        self.Update_Attack_riskcontrol_tree_Nodes(Parent_ID, control_head_node, node_map, attack_tree)
            elif Node_Type == 'riskcontrol leaf' or Node_Type == 'riskcontrol intermediate': pass
            else:
                if Node_Type == 'leaf': self.tree_parent_list.append(Parent_ID)
        ...
        # print(self.threat_node_map)
        DB.update_db('DELETE FROM attack_tree WHERE Node_ID LIKE ?', (f'{attack_tree}_node%',))
        for threat, node_info in self.threat_node_map.items():
                DB.update_db(f'''
                INSERT OR REPLACE INTO attack_tree (Node_ID, Parent_ID, Node_Type, Text, AF_Value, AF_Text, RF_Value, RF_Text, Gate_Type, "Values", "Image_Type", "x", "y")
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                        node_info["node_id"],
                        node_info["parent_id"],
    ...
    def Update_Attack_riskcontrol_tree_Nodes(self, parent_id, riskcontrol_node_id, node_map, attack_tree):
        node_info = node_map[riskcontrol_node_id]
        new_node_id = self.Add_riskcontrol_Child_Node(parent_id, node_info, attack_tree)
        self.riskcontrol_tree_added_list.append(new_node_id)
        # Recursively add child nodes
        for child_id in node_map[riskcontrol_node_id]["children"]:
                self.Update_Attack_riskcontrol_tree_Nodes(new_node_id, child_id, node_map, attack_tree)

    # Create a child node for the attack tree
    def Add_riskcontrol_Child_Node(self, parent_id, node_info, attack_tree):
        parent = self.threat_node_map[parent_id]
        child_id = f"{attack_tree}_node_{self.node_counter}"
        self.node_counter += 1
    ...
    def Calculate_AFR_Value(self):
        ...
        # for parent_id in self.tree_parent_list:
    ...
    def Get_Node_AF_Paths(self, node_id):
        ...
        for child_id in node["children"]:
            if "leaf" == self.threat_node_map[child_id]["node_type"]: 
    ...
    def Get_Node_RF_Paths(self, node_id):
        ...
        leaf_path_list = []
        for child_id in node["children"]:
            if "leaf" == self.threat_node_map[child_id]["node_type"] or "riskcontrol leaf" == self.threat_node_map[child_id]["node_type"]: 
                leaf_path_list.append(child_id)
            else:
    ...
    def Get_leafNode_Values(self, node_id):
        ...
        if "leaf" == node["node_type"] or "riskcontrol leaf" == node["node_type"]:
            values = []
    ...
    def Update_Node_Image(self, node_id, included):
        ...
        if included: node["image_type"] = "leaf_node_included"
        else: node["image_type"] = "leaf_node"
