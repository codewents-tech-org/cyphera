...
def display_selected_row(table, property_controls, property_panel, toggle_button):
    ...
    try:
        if table.selectedIndexes():  # Check if any row is selected
            ...
            toec_display(table, selected_row, property_controls)
        else:
            ...
            toec_display_reset(property_controls)
    except Exception as e:
        QMessageBox.critical(None, "Error", f"Error loading data: {e}")  # Show error message to the user

def toec_display(table, selected_row, toec_property_controls):
    try:
        toec_property_controls[0][1].setText(table.item(selected_row, 1).text())
        toec_property_controls[1][1].setText(table.item(selected_row, 2).text())
        toec_property_controls[2][1].setText(table.item(selected_row, 3).text())
        toec_property_controls[3][1].setText(table.item(selected_row, 4).text())
    except Exception as e:
        pass

def toec_display_reset(toec_property_controls):
    try:
        toec_property_controls[0][1].setText('')
        toec_property_controls[1][1].setText('')
        toec_property_controls[2][1].setText('')
        toec_property_controls[3][1].setText('')
    except Exception as e:
        pass
