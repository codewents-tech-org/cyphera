...
def find_duplicates(self, changed_item):
    ...
    self.refrash_existing_entries()
    self.existing_entries.discard(self.previous_text)
...
def refrash_existing_entries(self):
    ...
    for entry in available_scope_entrys: self.existing_entries.add(entry.strip())
    scope_existing_lower = {entry.lower() for entry in available_scope_entrys}
    existing_lower = {entry.lower() for entry in self.existing_entries}
    available_entry = []
    ...
    for entry in existing_lower: 
        if entry not in available_entry and entry not in scope_existing_lower: remove_entrys.append(entry)
