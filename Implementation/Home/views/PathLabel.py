...
class PathLabel(QLabel):
    def __init__(self, text, width=300, parent=None):  
        ...
        self.setToolTip(text)  # Set tooltip to show full path
        self.fixed_width = width  # Set fixed width for truncation

    def paintEvent(self, event):
        """Custom paint event to elide text when it overflows."""
        painter = QPainter(self)
        metrics = QFontMetrics(self.font())
        elided_text = metrics.elidedText(self.text(), Qt.ElideRight, self.fixed_width)  # End truncation
        painter.drawText(self.rect(), Qt.AlignLeft | Qt.AlignVCenter, elided_text)

    def sizeHint(self):
        """Override sizeHint to ensure correct label sizing."""
        metrics = QFontMetrics(self.font())
        return QSize(self.fixed_width, metrics.height())        

    def enterEvent(self, event):
        """ Show tooltip dynamically when mouse enters if text is clipped """
        metrics = QFontMetrics(self.font())
        if metrics.width(self.text()) > self.fixed_width:
