...
from utils.recent_projects_utils import add_or_update_project
...
class MainOpen_Panel(QWidget):
    ...
    def load_cloud_folders(self):
        ...
        try:
            if not P.ssh_transport or not P.ssh_sftp:
                P.ssh_transport, P.ssh_sftp = get_sftp()

            folders = list_cloud_folders_with_sftp(P.ssh_sftp)
    ...
    def on_cloud_folder_clicked(self, item):
        folder = item.text()
        P.selected_cloud_folder = folder
    ...
    def load_cloud_files(self, folder):
        ...
        try:
            ...
            files = list_cloud_files_with_sftp(P.ssh_sftp, folder)
            for file in files:
    ...
    def on_cloud_file_clicked(self, item):
        ...
        if file.endswith('.tara'):
            P.selected_cloud_file = file
            self.open_button.setEnabled(True)
            P.remote_project_path = f"{os.getenv('REMOTE_BASE_PATH')}/{P.selected_cloud_folder}/{P.selected_cloud_file}"
        else:
            P.selected_cloud_file = None
            self.open_button.setEnabled(False)
    ...
    def check_input_fields(self):
        if self.storage_mode == "local":
            ...
            self.open_button.setEnabled(bool(P.selected_cloud_file and P.remote_project_path))
    ...
    def Open_Project(self):
        ...
        if self.storage_mode == "local":
            P.project_storage_mode = "local"  # ✅ Explicit
            self.open_local_project()
        else:
            P.project_storage_mode = "cloud"  # ✅ Explicit
    ...
    def open_cloud_project(self, force_tara_path=None):
        import os, json
        from urllib.parse import quote_plus
        from utils.server_connection import get_db_env_vars, fetch_env_from_server, get_sftp
        from controllers.database import get_engine_and_session

        try:
            # Step 1: Get .tara file from cloud or force path
            if force_tara_path:
                local_tmp_path = force_tara_path
                print(f"🔍 [DEBUG][CLOUD] Re-opening local .tara: {local_tmp_path}")
            else:
                if not P.selected_cloud_folder or not P.selected_cloud_file:
                    print("❌ [ERROR] No selected cloud project to open.")
                    return
                remote_path = os.getenv("REMOTE_BASE_PATH") + f"/{P.selected_cloud_folder}/{P.selected_cloud_file}"
                local_tmp_path = os.path.join("tmp", P.selected_cloud_file)

                print(f"🔍 [DEBUG][CLOUD] Downloading .tara from: {remote_path} -> {local_tmp_path}")
                if not P.ssh_transport or not P.ssh_sftp:
                    P.ssh_transport, P.ssh_sftp = get_sftp()
                P.ssh_sftp.get(remote_path, local_tmp_path)

            # Step 2: Parse .tara content
            with open(local_tmp_path, "r") as f:
                tara_data = json.load(f)
                print(f"📦 [DEBUG][CLOUD] .tara contents: {tara_data}")
                db_name = tara_data.get("project_name") or tara_data.get("db_name")
                P.Project_name = db_name
                P.current_db_name = db_name
                P.project_path = tara_data.get("project_path")
                if "db_url" in tara_data:
                    P.SQLALCHEMY_DATABASE_URL = tara_data["db_url"]

            if not db_name:
                print("❌ [ERROR] .tara missing DB name.")
                return

            # Step 3: Load .env from server only if db_url is missing
            if not tara_data.get("db_url"):
                print("🌐 [DEBUG] Fetching remote .env file...")
                if not P.ssh_transport or not P.ssh_sftp:
                    P.ssh_transport, P.ssh_sftp = get_sftp()
                ...
                print(f"🔗 [DEBUG][CLOUD] SQLAlchemy URL set from .env: {db_url}")
            else:
                print(f"🔗 [DEBUG][CLOUD] SQLAlchemy URL loaded from .tara: {P.SQLALCHEMY_DATABASE_URL}")

            # Step 4: Clear tree tabs
            interfaces.tool_reset_enable = True
            for Tree, (tree_tab, instances) in interfaces.tree_tab_panel.items():
                for i in reversed(range(tree_tab.count())):
                    instances.pop(f'{tree_tab.tabText(i)}', None)
            ...
            # Step 7: Set default selections
            helper.Panel_selector = 'ModuleTab'
            helper.modulePanel_Selector = 'TargetOfEvaluationTab'
            helper.submodulePanel_Selector = 'SystemDescriptionTab'
            interfaces.project_path = P.project_path or local_tmp_path
            ...
            # Step 8: Reset file input and stat
            add_or_update_project(
                name=db_name,
                path=local_tmp_path,
                mode="cloud",
                db_url=P.SQLALCHEMY_DATABASE_URL
            )

            print("✅ [DEBUG][CLOUD] Cloud project opened successfully.")

        except Exception as e:
            print(f"❌ [UNHANDLED ERROR] While opening cloud project: {e}")
