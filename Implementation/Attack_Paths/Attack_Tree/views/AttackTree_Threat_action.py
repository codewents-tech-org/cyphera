...
Purpose:
--------
Provides the UI logic for creating and displaying a Attack Tree tab using PyQt5.
It sets up the graphics view, loads data from the database, and interfaces with the 
AttackTreeScene and its node/arrow layout system.

Description:
------------
This module defines the `TechnicalATClass`, a QWidget subclass that represents a
dynamic, graphical, and interactive Attack Tree editor. It embeds a 
...
from Attack_Paths.Attack_Tree.controllers.attacktree_scene import AttackTreeScene
...
from controllers.tablemodel import NodeType, ReferenceTrees, AttackTree, AttackIntermediateNodes, AttackLeafNodes
...
class ThreatATClass(QWidget):
    """
    Widget class that encapsulates a Attack Tree editor tab.

    This class handles the graphical view and scene setup, data loading,
    and tree initialization for a specific Attack tree ID.
    """
    def __init__(self, parent=None):
        """
        Initializes the attack tree editor.
    ...
    def Create_AttackTree_Tab(self, threat_id, threat_name):
        """
        Creates a new tab (or reuses an existing one) for the attack tree.

        Args:
            threat_id (str): Unique ID of the attack tree.
            threat_name (str): Display name or description.
        """
    ...
    def Setup_GraphicsScene(self):
        ...
        self.scene = AttackTreeScene(self.threat_id)
    ...
    def Save_Tree(self): 
        """
        Persists the current attack tree structure to internal state or disk.
    ...
    def Load_AttackTree(self):
        """
        Loads a attack tree from the database or fallback template.
        ...
        try:
            ...
            at_nodes = get_instances(AttackTree, {'tree_id':self.threat_id, 'is_deleted':False})

            if at_nodes:
                at_json_tree = build_at_tree_json(at_nodes)
                self.scene.load_tree(at_json_tree)
            else: 
                self.scene.initialize_parameters()
                self.scene.add_head_node(f"{self.threat_id}_node_0", self.threat_id, self.threat_name, 0, 0, "0", "High", "", "", "AND")
                
            interfaces.previous_tree = self
            interfaces.unsaved_changes = True
            logger.info(f"Attack tree loaded successfully for {self.threat_id} threat")
        except Exception as e:
            logger.info(f"Load AttackTree failed for {self.threat_id} threat")
            self.scene.initialize_parameters()
            self.scene.add_head_node(f"{self.threat_id}_node_0", self.threat_id, self.threat_name, 0, 0, "0", "High", "", "", "AND")
            interfaces.previous_tree = self
            interfaces.unsaved_changes = True
