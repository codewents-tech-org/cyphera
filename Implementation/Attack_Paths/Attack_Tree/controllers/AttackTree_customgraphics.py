...
class CustomGraphicsRemoveItem(QGraphicsProxyWidget):
    def __init__(self, node_id, parent=None, attack_tree_parent=None):
        super().__init__(parent)
        logger.info(f"CustomGraphicsRemoveItem: {node_id}")
        self.node_id = node_id
        self.attack_tree_parent = attack_tree_parent

    def contextMenuEvent(self, event):
        logger.info(f"CustomGraphicsRemoveItem: {self.node_id}")
        context_menu = QMenu()
        
        switch_action = QAction("remove node")
        switch_action.triggered.connect(lambda: self.attack_tree_parent.Remove_Node(self.node_id))
        context_menu.addAction(switch_action)
        
        context_menu.exec_(event.screenPos())

class EditableTextItem(QGraphicsTextItem):
    def __init__(self, text='', parent=None, fixed_width=200, node_id='', attack_tree_parent=None):
        logger.info(f"EditableTextItem: {node_id}")
    ...
    def update_display_text(self):
        logger.info(f"EditableTextItem: {self.node_id}")
    ...
    def focusOutEvent(self, event):
        ...
        if new_text != self.full_text:  # Check if text has changed
            ...
            self.update_display_text()
            if self.attack_tree_parent is not None:
                interfaces.unsaved_changes = True
                self.attack_tree_parent.Auto_Update_Leaf_name(self.node_id)
    ...
    def boundingRect(self, height = ''):
        ...
        return QRectF(rect.x(), rect.y(), self.fixed_width, 45)

class CustomGraphicsEditItem(QGraphicsProxyWidget):
    def __init__(self, node_id, parent=None, attack_tree_parent=None):
        super().__init__(parent)
        logger.info(f"CustomGraphicsEditItem: {node_id}")
        self.node_id = node_id
        self.attack_tree_parent = attack_tree_parent

    def contextMenuEvent(self, event):
        logger.info(f"CustomGraphicsEditItem: {self.node_id}")
        # Create a context menu
        menu = QMenu()
        
        intermediatenode_action = QAction("Add Intermediate Node", menu)
        intermediatenode_action.triggered.connect(lambda: self.attack_tree_parent.Add_Child_Node(self.node_id, "intermediate"))
        ...
        add_newleaf_action.triggered.connect(lambda: self.attack_tree_parent.Add_Child_Node(self.node_id, "new_leaf"))
        ...
        # Add each saved leaf node as an action
            ...
            for leaf_name in saved_leaf_nodes:
                ...
                leaf_action.triggered.connect(lambda checked, name=leaf_name: self.attack_tree_parent.Add_Child_Node(self.node_id, "existing_leaf", name ))
        ...
        add_existingriskcontrol_menu = QMenu("Add Risk Control Tree")
        saved_riskcontrol_tree = get_existing_riskcontrol_tree()
        
        # If no saved Risk Control Tree are found, disable the option
        if not saved_riskcontrol_tree:
            no_RiskControlTree_action = QAction("Risk Control Trees not available", add_existingriskcontrol_menu)
            no_RiskControlTree_action.setEnabled(False)
            add_existingriskcontrol_menu.addAction(no_RiskControlTree_action)
        # Add each saved Risk Control Tree as an action
        else:
            for riskcontrol_name in saved_riskcontrol_tree:
                riskcontrol_action = QAction(riskcontrol_name, add_existingriskcontrol_menu)
                riskcontrol_action.triggered.connect(lambda checked, name=riskcontrol_name: self.attack_tree_parent.Add_riskcontrol_tree(self.node_id, name))
                add_existingriskcontrol_menu.addAction(riskcontrol_action)
        menu.addMenu(add_existingriskcontrol_menu)
        ...
        # Add each saved technical tree as an action
        else:
            for technical_name in saved_technical_tree:
                technical_action = QAction(technical_name, add_existingtechnical_menu)
                technical_action.triggered.connect(lambda checked, name=technical_name: self.attack_tree_parent.Add_Technical_Tree(self.node_id, name))
        ...
        removenode_action.triggered.connect(lambda: self.attack_tree_parent.Remove_Node(self.node_id))
...
class CustomGraphicsNonEditItem(QGraphicsProxyWidget):
    def __init__(self, node_id, parent=None, attack_tree_parent=None):
        super().__init__(parent)
        logger.info(f"CustomGraphicsNonEditItem: {node_id}")
        self.node_id = node_id
        self.attack_tree_parent = attack_tree_parent

    def contextMenuEvent(self, event):
        logger.info(f"CustomGraphicsNonEditItem: {self.node_id}")
        # Create a context menu
        menu = QMenu()
...
class AFR_level(QGraphicsTextItem):
    ...
    def update_display_text(self):
        logger.info(f"AFR_level: {self.full_text}")
...
class SidebarItem(QGraphicsTextItem):
    def __init__(self, text='', bg_color='white'):
        super().__init__()
        logger.info(f"SidebarItem: {text}")
        # Set fixed width for the item
    ...
    def update_bg_color(self, color_bg):
        logger.info(f"SidebarItem: {color_bg}")
    ...
    def update_display_text(self):
        logger.info(f"SidebarItem: {self.full_text}")
        # Use font metrics to calculate whether truncation is needed
        font_metrics = QFontMetrics(self.font())
        elided_text = font_metrics.elidedText(self.full_text, Qt.ElideRight, 90, 0)
        super().setPlainText(elided_text)
...
class CustomGraphicsTextItem(QGraphicsTextItem):
    def __init__(self, text, width=100, height=50, parent=None, font=None):
        super().__init__(text, parent)
        logger.info(f"CustomGraphicsTextItem: {text}")
...
class CustomComboBox(QComboBox):
    def __init__(self, *args, **kwargs):
        ...
        logger.info(f"CustomComboBox: {args}")
...
def get_existing_leaf_nodes():
    logger.info("get_existing_leaf_nodes")
    try:
        leaf_rows = get_instances(AttackLeafNodes)
        if not leaf_rows:
            return []
        return [f"{row.id} {row.name}" for row in leaf_rows]
    except Exception as e:
        logger.error(f"Error in get_existing_leaf_nodes: {e}")
        return []


def get_existing_riskcontrol_tree():
    logger.info("get_existing_riskcontrol_tree")
    try:
        riskcontrol_rows = get_instances(RiskControlTreeHome)
        if not riskcontrol_rows:
            return []
        return [f"{row.rc_id} {row.name}" for row in riskcontrol_rows]
    except Exception as e:
        logger.error(f"Error in get_existing_riskcontrol_tree: {e}")
        return []


def get_existing_technical_tree():
    logger.info("get_existing_technical_tree")
    try:
        technical_rows = get_instances(TechnicalTreeHome)
        if not technical_rows:
            return []
        return [f"{row.id} {row.name}" for row in technical_rows]
    except Exception as e:
        logger.error(f"Error in get_existing_technical_tree: {e}")
        return []
