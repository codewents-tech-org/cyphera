"""
Module: Attack Tree Scene      \n 
File: attacktree_scene.py      \n
...
Purpose:
--------
Defines the `AttackTreeScene` class for managing and rendering a dynamic risk control tree 
...
Description:
------------
The `AttackTreeScene` class extends `QGraphicsScene` to support visual construction of 
...
from Attack_Paths.controllers.highlight_afr_path import create_arrow_for_nodes, update_arrow_lines, create_arrow_lines
...
class AttackTreeScene(QGraphicsScene):
    """
    Manages the rendering and interaction of a Attack Attack Tree using QGraphicsScene.

    This class handles creation, linking, layout, highlighting, and data update
    operations for risk control tree nodes such as head, intermediate, and leaf types.
    """
    def __init__(self, tat_id):
        """
        Initializes a new risk control attack tree scene.
    ...
    def add_head_node(self, tree_id: str="unknown", label: str="unknown", text: str="unknown", 
        ...
        """
        head_node = {   "tree_id": tree_id, "node_label": label, "node_Text": text, "af_value": af_value, "af_level": af_level, "rf_value": rf_value, "rf_level": rf_level, "x": x, "y": y, "gate_type": gate}
        node = headNodeBox(head_node, tree_type="AttackTree")
        self.addItem(node)
        ...
        node.addTechnicalTreeRequested.connect(self.import_tree)
        node.addRiskControlTreeRequested.connect(self.import_tree)
    ...
    def add_intermediate_node(self, tree_id: str="unknown", label: str="unknown", text: str="unknown", 
        ...
        node = IntermediateNodeBox(intermediate_node, tree_type="AttackTree")
        self.addItem(node)
        node.addIntermediateRequested.connect(self.Add_IntermediateNode_Request)
        node.addLeafRequested.connect(self.Add_LeafNode_Request)
    ...
    def add_treehead_node(self, tree_id: str="unknown", label: str="unknown", text: str="unknown", 
                              x: int=0, y: int=0, gate: str='AND', parent_id: str=None, tree_type: str="unknown")-> dict:
        """
        Adds an tree head node under a given parent.

        Args:
            tree_id (str): Unique ID for the tree head node.
            label (str): Node label.
            text (str): Node description.
            x (int): X position.
            y (int): Y position.
        ...
        Returns:
            dict: Serialized tree head node dictionary.
        """
        treehead_node = {   "tree_id": tree_id, "node_label": label, "node_Text": text, "x": x, "y": y, "gate_type": gate, "tree_type": tree_type}
        node = TreeHeadNodeBox(treehead_node, tree_type="AttackTree")
        self.addItem(node)
        node.removeRequested.connect(self.remove_node_Request)
        node_type = "control head" if tree_type == "RiskControlTree" else "technical head"
        child_node = {"node_type": node_type, "node_id": tree_id, "node": node}
        self.update_tree_json(self.tree_nodes, parent_id, child_node)
        return child_node
    ...
    def add_leaf_node(self, tree_id: str="unknown", label: str="unknown", text: str="unknown", 
        ...
        node = LeafNodeBox(leaf_node, tree_type="AttackTree")
        self.addItem(node)
    ...
    def Add_IntermediateNode_Request(self, node_data:dict) -> None:
        ...
        print(f"Adding Intermediate Node with ID: {node_label}, \n last_intermediate_node_id: {self.last_intermediate_node_id}")
    ...
    def Add_LeafNode_Request(self, node_info:dict) -> None:
        ...
        print(f"Adding Leaf Node with ID: {node_data['node_label']}, \n last_leaf_node_id: {self.last_leaf_node_id}")
    ...
    def import_tree(self, parent_info: dict, tree_type: str) -> None:
        """
        Delegate technical tree import to TreeImporter instance.

        Args:
            parent_info (dict): Information of the parent node.
        """
        importer = TreeImporter(self)
        importer.import_tree(parent_info, tree_type)
    
    def data_process_operation(self) -> None:
        """
        Recalculates root AFR path and updates arrow highlights.
        """
        output = head_node_afr_update(self.tree_nodes, "attack_tree")
        print(output)
    ...
    def load_tree(self, tree_dict: dict) -> None:
        ...
        """
        load_tree_items_with_data(self=self, tree_dict=tree_dict, tree_type="attack_tree")
    ...
    def save_tree(self) -> dict:
        ...
        """
        tree_data = get_tree_data({"tree":self.tree_nodes, "tree_type":"attack_tree"})
        save_tree_in_database(self.tat_id, tree_data, "attack_tree")
        
        update_status = update_afr_values(self.updated_leaf_list, self.tat_id, "attack_tree")
        if update_status:
            self.updated_leaf_list = []
