"""
Module: RiskControl Tree Scene      \n 
File: riskcontroltree_scene.py      \n
...
Purpose:
--------
Defines the `RiskControlTreeScene` class for managing and rendering a dynamic risk control tree 
...
Description:
------------
The `RiskControlTreeScene` class extends `QGraphicsScene` to support visual construction of 
...
class RiskControlTreeScene(QGraphicsScene):
    """
    Manages the rendering and interaction of a RiskControl Attack Tree using QGraphicsScene.
    ...
    def add_head_node(self, tree_id: str="unknown", label: str="unknown", text: str="unknown", 
        ...
        node = headNodeBox(head_node, tree_type="RiskControlTree")
        self.addItem(node)
    ...
    def add_intermediate_node(self, tree_id: str="unknown", label: str="unknown", text: str="unknown", 
        ...
        node = IntermediateNodeBox(intermediate_node, tree_type="RiskControlTree")
        self.addItem(node)
        node.addIntermediateRequested.connect(self.Add_IntermediateNode_Request)
        node.addLeafRequested.connect(self.Add_LeafNode_Request)
    ...
    def add_treehead_node(self, tree_id: str="unknown", label: str="unknown", text: str="unknown", 
        ...
        node = TreeHeadNodeBox(treehead_node, tree_type="RiskControlTree")
        self.addItem(node)
        node.removeRequested.connect(self.remove_node_Request)
        node_type = "control head" if tree_type == "control_tree" else "technical head"
        child_node = {"node_type": node_type, "node_id": tree_id, "node": node, "is_new": False}
        self.update_tree_json(self.tree_nodes, parent_id, child_node)
        return child_node
    ...
    def add_leaf_node(self, tree_id: str="unknown", label: str="unknown", text: str="unknown", 
        ...
        node = LeafNodeBox(leaf_node, tree_type="RiskControlTree")
        self.addItem(node)
    ...
    def data_process_operation(self) -> None:
        ...
        """
        output = head_node_afr_update(self.tree_nodes, "control_tree")
    ...
    def load_tree(self, tree_dict: dict) -> None:
        ...
        """
        load_tree_items_with_data(self=self, tree_dict=tree_dict, tree_type="control_tree")
    ...
    def remove_node_Request(self, node_id: str) -> None:
        ...
        print(node_id, " -> ", self.extracted_tree)
        if removed:
            self.tree_layout.apply_layout_positions(self.tree_nodes)
            self.data_process_operation()
    ...
    def save_tree(self) -> dict:
        ...
        """
        tree_data = get_tree_data({"tree":self.tree_nodes, "tree_type":"risk_control_tree"})
        save_tree_in_database(self.tat_id, tree_data, "risk_control_tree")
        
        update_status = update_afr_values(self.updated_leaf_list, self.tat_id, "risk_control_tree")
        if update_status:
            self.updated_leaf_list = []
