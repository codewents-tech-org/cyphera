...
class RiskControlTree_Update():
    def __init__(self, tree):
        logger.info(f"Updating Risk Control Tree: {tree}")
        available_tree = DB.execute_db(f"SELECT * FROM riskcontrol_tree WHERE Node_ID like '{tree}_node%'")
        if available_tree:
    ...
    def Calculate_AFR_Value(self):
        logger.info("Calculating AFR Value")
    ...
    def update_root_node_value(self, thread_head_id):
        logger.info(f"Updating Root Node Value: {thread_head_id}")
    ...
    def update_root_node_RF_value(self, head_node):
        ...
        if possible_paths:
            ...
            path_leaf_list = ''
            path_min_value = 0
        # print('riskcontrol output data: ', path_leaf_list, '\t', path_min_value)
        ...
        for node, node_info in self.control_node_map.items():
            if node_info["node_type"] in ["leaf", "technical leaf"]:
    ...
    def Get_Node_AF_Paths(self, node_id):
        ...
        for child_id in node["children"]:
            if self.control_node_map[child_id]["node_type"] in ["leaf", "technical leaf"]: 
    ...
    def Get_leafNode_Values(self, node_id):
        ...
        if node["node_type"] in ["leaf", "technical leaf"]:
            values = []
