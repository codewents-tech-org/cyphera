...
def assum_delete_entry(table):
    logger.info("Assumption deletion")
    try:
        ...
        if replay == QMessageBox.Ok:
            ...
            DB.update_db("DELETE FROM assumptions WHERE assumption_id = ?", (id_item,))
            DB.update_db("INSERT INTO assum_trash (id) VALUES (?)", (id_item,))
...
def toec_delete_entry(table):
    logger.info("toec deletion")
    try:
        replay = QMessageBox.warning(None, "Warning", "Delete selected row.", QMessageBox.Ok|QMessageBox.Cancel, QMessageBox.Ok)
        if replay == QMessageBox.Ok:
            selected_row = table.currentRow()
            if selected_row < 0:
                ...
                return
            id_item = table.item(selected_row, 1).text()
            DB.update_db("DELETE FROM toe_configuration WHERE toe_configuration_id = ?", (id_item,))
            DB.update_db("INSERT INTO trash_toe (id) VALUES (?)", (id_item,))
            table.removeRow(selected_row)
            row_count = table.rowCount()
            if row_count > 0:
                next_row = min(selected_row, row_count - 1)
